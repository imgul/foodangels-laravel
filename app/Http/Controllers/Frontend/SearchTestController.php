...
class SearchTestController extends FrontendController
{
...
//        dd("good");
        //$branches = Branch::selectRaw('*, IF((opening_time < "' . now()->format('H:i') . '" and closing_time > "' . now()->format('H:i') . '"), true, false) as open,( 3959 * acos( cos( radians( '.$lat.' ) ) * cos( radians( lat ) ) * cos( radians( long ) - radians( '.$lng.' ) ) + sin( radians( '.$lat.') ) * sin( radians( lat ) ) ) ) as distance')
        $branches = Restaurant::selectRaw('*, IF((opening_time < "' . now()->format('H:i') . '" and closing_time > "' . now()->format('H:i') . '"), true, false) as open,lat,`long`')
            ->where(function ($query) use ($request, $current_time) {
                ...
                if (!blank($request->get('minimumOrder'))) {
                    if ($request->get('minimumOrder') != "all") {
                        ...
                        $query->whereHas('branchPostalCode', function ($querys) use ($minimumOrder, $code) {
                ...
                if (!blank($request->get('postal_code'))) {
                    $code = $request->get('postal_code');
                    $query->whereHas('branchPostalCode', function ($queryPostal) use ($code) {
                ...
                if (!blank($request->get('delivery_free'))) {
                    if ($request->get('delivery_free') == 'true') {
                        $query->whereHas('branchPostalCode', function ($queryDelivery) {
        ...
        $is_pickup =true;
        $is_delivery =true;
        if (!blank($branches->get())) {
            foreach ($branches->get() as $value) {
        ...
        foreach ($branches->get() as $key => $branch) {
            $ratingArray                      = $rating->avgRating($branch->id);
            $totalReview[$branch->id]       = $ratingArray['countUser'];
            $averageRating[$branch->id]       = $ratingArray['avgRating'];
            $mapbranches[$key]['name']      = $branch->name;
            $mapbranches[$key]['slug']      = $branch->slug;
            $mapbranches[$key]['image']     = $branch->image;
            $mapbranches[$key]['logo']      = $branch->logo;
            $mapbranches[$key]['avgRating'] = $ratingArray['avgRating'];
            $mapbranches[$key]['countUser'] = $ratingArray['countUser'];
            $mapbranches[$key]['lat']       = $branch->lat;
            $mapbranches[$key]['long']      = $branch->long;
            $mapbranches[$key]['address']   = $branch->address;
            $mapbranches[$key]['url']       = route('restaurant.show', [$branch]);
        }
        $this->data['is_closed']     = false;
        $this->data['is_postalCode'] = $is_postalCode;
        $this->data['branches']      = $branches->paginate(8)->appends(request()->query());
        $this->data['totalReview']   = $totalReview;
        $this->data['averageRating'] = $averageRating;
        //return view('frontend.search', $this->data);
        $Branch = Restaurant::where('slug', 'foodangels')->first();
        $this->data['featured']=  MenuItem::with('media')->where(['branch_id'=>$Branch->id,'is_featured'=>1])->get();
        $this->data['branch']     = $Branch;
        $opening_time = $Branch->week_days_opening;
        $closing_time = $Branch->week_days_closing;
...
// echo "<pre>"; print_r( $this->data['is_closed']); die;
        ...
        $weekend = [];
        $period = new CarbonPeriod($Branch->weekend_opening, '30 minutes', $Branch->weekend_closing); // for create use 24 hours format later change format
        foreach($period as $item){
            array_push($weekend,$item->format("h:i A"));
        }
        $this->data['weekend']     = $weekend;
        // echo "<pre>"; print($branches->get()->count());die;
        Session::put('branch_d_type', false);
        Session::put('delivery_type', DeliveryType::DELIVERY);
        if ($branches->get()->count() != 0 && $branches->get()->count() <= 1 && $request->get('homepage')) {

            return redirect(route('restaurant.show', [$mapbranches[0]['slug']]));
        } else {
            return view('frontend.search', $this->data);
        }
    }
