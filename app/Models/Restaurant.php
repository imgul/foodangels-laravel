...
class Restaurant extends BaseModel implements HasMedia
{
    ...
    public function restaurantPostalCode()
    {
        return $this->hasOne(RestaurantPostalCode::class);
    }

    public function postalCode()
    {
        return RestaurantPostalCode::where(['restaurant_id'=>$this->id])->first();
        // 'postal_code'=>session('postal_code')
    }

    // get the postal code which has the lowest delivery charges
    public function lowestDeliveryCharge()
    {
        return RestaurantPostalCode
            ::where('restaurant_id', $this->id)
            ->orderBy('delivery_charge', 'asc')
            ->first();
    }
