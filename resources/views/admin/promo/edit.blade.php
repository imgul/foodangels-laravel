	...
	<div class="section-body">
		<div class="row">
			<div class="col-12 col-md-12 col-lg-12">
				<div class="card">
					<form action="{{ route('admin.promo.update', $promo) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
						@csrf
						@method('PUT')
						<div class="card-body">
							<div class="form-row">
								...
								<div class="form-group col">
									<label for="area">{{ __('levels.branch') }}</label> <span class="text-danger">*</span>
									<select name="restaurant_id" id="area" class="select2 form-control @error('restaurant_id') is-invalid red-border @enderror">
										...
										<option value="{{ $restaurant->id }}" {{ (old('restaurant_id',$promo->restaurant_id) == $restaurant->id) ? 'selected' : '' }}>{{ $restaurant->name }}
										</option>
										@endforeach
										@endif
									</select>
								...
								<div class="form-group col">
									...
									<input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name',$promo->name) }}">
									@error('name')
									<div class="invalid-feedback">
										{{ $message }}
									</div>
							...
							<div class="form-row">
								<div class="form-group col">
									...
									<div class="input-group">
										<input type="number" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount',$promo->amount) }}" aria-describedby="basic-addon2">
								...
								<div class="form-group col">
									...
									<div class="input-group">
										<input type="number" class="form-control @error('user_limit') is-invalid @enderror" name="user_limit" value="{{ old('user_limit',$promo->user_limit) }}" aria-describedby="basic-addon2">
										<div class="input-group-append">
										  <span class="input-group-text bg-secondary" id="basic-addon2"><i class="fas fa-percentage"></i></span>
										</div>
									</div>
								...
								<div class="form-group col">
									...
									<select name="status" class="form-control @error('status') is-invalid @enderror">
										...
										<option value="{{ $key }}" {{ (old('status',$promo->status) == $key) ? 'selected' : '' }}>{{ $status }}</option>
										@endforeach
									</select>
									@error('status')
									<div class="invalid-feedback">
							...
							<div class="form-row">
								<div class="form-group col">
									...
									<input type="text" name="from_date" class=" datepicker form-control @error('from_date') is-invalid @enderror" value="{{ old('from_date',$promo->from_date) }}">
									@error('from_date')
									<div class="invalid-feedback">
										{{ $message }}
									</div>
								...
								<div class="form-group col">
									...
									<input type="text" name="to_date" class="form-control datepicker @error('to_date') is-invalid @enderror" value="{{ old('to_date',$promo->to_date) }}">
									@error('to_date')
									<div class="invalid-feedback">
										{{ $message }}
									</div>
...
<script src="{{ asset('js/promo/edit.js') }}"></script>
@endsection
